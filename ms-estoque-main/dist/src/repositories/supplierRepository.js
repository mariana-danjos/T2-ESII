...
class SupplierRepository {
    ...
    async create(input) {
        ...
        const { rows } = await this.pool.query(`INSERT INTO suppliers
       ...
       RETURNING *`, [input.legalName, input.tradeName ?? null, input.document, input.documentType,
    ...
    async update(id, input) {
        const a = input.address ?? {};
        const { rows } = await this.pool.query(`UPDATE suppliers SET
         ...
         updated_at = now()
       WHERE id = $1
       RETURNING *`, [id, input.legalName ?? null, input.tradeName ?? null, input.email ?? null,
    ...
    async softDelete(id) {
        const { rows } = await this.pool.query(`UPDATE suppliers SET status = 'inactive', updated_at = now() WHERE id = $1 RETURNING *`, [id]);
        return rows[0] ? mapRow(rows[0]) : null;
    }
    async list(query) {
        const where = [];
        ...
        const countRes = await this.pool.query(`SELECT count(*)::int AS total FROM suppliers ${whereSql}`, params);
        const total = countRes.rows[0].total;
        const offset = (query.page - 1) * query.pageSize;
        const dataRes = await this.pool.query(`SELECT * FROM suppliers ${whereSql} ORDER BY created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [...params, query.pageSize, offset]);
